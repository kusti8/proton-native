...
var Window = function (_DesktopComponent) {
  ...
  function Window(root, props) {
    ...
    _this.element = new _libuiNode2.default.UiWindow(_this.props.title, _this.props.size.w, _this.props.size.h, _this.props.menuBar);
    _this.element.onClosing(function () {
      _this.props.onClosing();
      _this.element.close();
      if (_this.props.lastWindow) {
        _libuiNode2.default.stopLoop();
      }
    });
    _this.element.margined = _this.props.margined;
    _this.element.position.x = _this.props.position.x;
    _this.element.position.y = _this.props.position.y;
    _this.element.fullscreen = _this.props.fullscreen;
    _this.element.borderless = _this.props.borderless;

    if (_this.props.centered) {
      _this.element.center();
    }

    _this.element.onPositionChanged(function () {
      _this.props.onPositionChanged({
        x: _this.element.position.x,
        y: _this.element.position.y
      });
    });
    _this.element.onContentSizeChanged(function () {
      _this.props.onContentSizeChanged({
        h: _this.element.position.h,
        w: _this.element.position.w
      });
    });
    return _this;
  }
